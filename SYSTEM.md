...
> Runtime baseline ที่ยืนยันแล้วบนเครื่องใช้งานจริง: Ubuntu 22.04.5 LTS, Docker 28.4.0, Docker Compose v2.39.1, host Node/npm = v12.22.9 / 8.5.1, current live stack = `docker-compose.yml` (development profile) + Cloudflare Tunnel → `http://localhost:5173`
...
ปัจจุบัน public traffic ของเครื่องจริงเข้าเว็บผ่าน `cloudflared` systemd service แล้ว tunnel ไปที่ `localhost:5173` ซึ่งคือ frontend Vite dev server ใน `docker-compose.yml`

Dev mock seed data ถูกแยกออกจาก `server.js` ไปที่ `backend/src/seeds/mockData.js` และถูกเรียกใช้ตอน bootstrap เท่านั้น

สำหรับ local/demo ที่ต้องการ build images แล้วรันทันที ตอนนี้มี `docker-compose.quickstart.yml` ซึ่งรัน `postgres` + `mock-provider` + one-shot `migrate` + `backend` + `frontend` ใน stack เดียว โดย frontend เปิดที่ `http://localhost:8080` และ mock payment page เปิดที่ `http://localhost:4001`

quickstart stack ใช้ `database/schema.sql` เป็น bootstrap schema สำหรับ PostgreSQL volume ใหม่, จากนั้น one-shot `migrate` service จะตรวจ imported schema แล้ว mark migration files ปัจจุบันเป็น applied ก่อน start `backend`; frontend ยังใช้ `frontend/Dockerfile` (production build + Nginx) แต่ inject `NODE_ENV=development` ให้ `migrate`/`backend` เพื่อคง mock-first flow (`PAYMENT_PROVIDER=mock`, OTP debug, demo seed) และชี้ `MOCK_PROVIDER_BASE_URL` ไปที่ `http://mock-provider:4000`
...
> Source: `database/schema.sql` (1474 lines, 41 tables)
...
> Source of truth templates: `/home/careconnect/Careconnect/.env.example`, `/home/careconnect/Careconnect/.env.quickstart.example`, และ `/home/careconnect/Careconnect/.env.production.example`
...
> Docker note: ใน `docker-compose.yml` / `docker-compose.quickstart.yml` / `docker-compose.prod.yml` ค่า env ถูก inject เข้า container โดยตรง ดังนั้น root `.env` ถูกอ่านโดย Docker Compose ฝั่ง host เป็นหลัก ไม่ใช่จาก path ใน container
> Dev compose ปัจจุบัน inject ค่าอย่าง `DATABASE_HOST=postgres`, `WEBHOOK_BASE_URL=http://backend:3000`, `UPLOAD_DIR=/app/uploads`, `MOCK_PROVIDER_BASE_URL=http://mock-provider:4000`; quickstart compose inject `NODE_ENV=development`, `FRONTEND_URL=http://localhost:8080`, `BACKEND_URL=http://localhost:8080`, `MOCK_PROVIDER_BASE_URL=http://mock-provider:4000`, `WEBHOOK_BASE_URL=http://backend:3000`, และให้ `mock-provider` รับ `MOCK_PROVIDER_PUBLIC_URL=http://localhost:4001` เพื่อสร้าง browser-facing payment link
> Google OAuth note: โค้ดปัจจุบันไม่อ่าน `GOOGLE_CALLBACK_URL`; callback จะอิง public frontend origin / `FRONTEND_URL` แล้ววิ่งผ่านเส้น `/api/auth/google/callback`
...
> Quickstart compose-only env: `.env.quickstart.example` เพิ่ม `HTTP_PORT`, `MOCK_PROVIDER_PORT`, และ `MOCK_PROVIDER_PUBLIC_URL` สำหรับ host port mapping (`8080`, `4001`) และ public mock payment link; file เดียวกันนี้ถูกใช้ทั้งกับ one-shot `migrate` service และ `backend` service ใน `docker-compose.quickstart.yml`

### Frontend (`VITE_*`)

> Local non-Docker: `frontend/vite.config.ts` merge `VITE_*` จาก root project และโฟลเดอร์ `frontend/` ตาม mode ที่ Vite โหลดได้
> Docker development: `docker-compose.yml` inject `VITE_*` เข้า frontend container โดยตรง
> Docker production/quickstart: `docker-compose.prod.yml` และ `docker-compose.quickstart.yml` ส่ง `VITE_*` เข้า `frontend/Dockerfile` ผ่าน build args ก่อน `npm run build`
> Public-access note: `VITE_PUBLIC_HOST`, `VITE_PUBLIC_PROTOCOL`, `VITE_PUBLIC_PORT`, `VITE_PUBLIC_HMR_PORT` ใช้สำหรับ dev HMR เมื่อ frontend ถูกเปิดผ่าน public hostname/Cloudflare Tunnel
