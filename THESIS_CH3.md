...
│  PostgreSQL 15 / 31 tables│  │  Payment/SMS/KYC Port 4000 │
└───────────────────────────┘  └────────────────────────────┘
```
...
| `migrate`       | —    | Database migration (on-demand profile)    |
...
**Route Guards**: `RequireAuth`, `RequireRole`, `RequirePolicy`, `RequireProfile`, `RequireAdmin`
...
| Webhook & Provider  | provider_webhooks                                            |
...
| Axios            | 1.6     | HTTP client สำหรับเรียก REST API      |
| Socket.IO Client | 4.6     | Real-time WebSocket (Chat/Notification)|
| react-hot-toast  | 2.4     | Toast notification แจ้งเตือน          |
| Lucide React     | 0.303   | Icon library (SVG icons)              |
| Leaflet + React-Leaflet | 1.9 / 4.2 | แผนที่แสดงตำแหน่งงาน       |
| zustand          | 4.4     | Lightweight state management          |
| zod              | 3.22    | Schema validation (frontend forms)    |
...
| Socket.IO       | 4.6     | WebSocket server (Chat/Notification)  |
| jsonwebtoken    | 9.0     | JWT generation + verification         |
| bcrypt          | 5.1     | Password hashing (bcrypt)             |
| Joi             | 17.11   | Request body/query/params validation  |
| pg              | 8.11    | PostgreSQL client (node-postgres)     |
| multer          | 1.4     | Multipart file upload (avatar, KYC)   |
| google-auth-library | 9.15 | Google OAuth 2.0 verification        |
| express-rate-limit | 7.1  | Rate limiting (auth, OTP, webhook)    |
| nodemailer      | 6.9     | Email sending (password reset)        |
| sharp           | 0.33    | Image processing (resize, compress)   |
| winston         | 3.11    | Structured logging                    |
| stripe          | 14.21   | Stripe payment integration (optional) |
| node-cron       | 3.0     | Scheduled tasks (cron jobs)           |
| exif-parser     | 0.1     | Photo EXIF metadata extraction        |
...
| `/register`          | RegisterTypePage   | เลือกประเภทการสมัคร         |
...
| `/reset-password`    | ResetPasswordPage  | ตั้งรหัสผ่านใหม่ (token)    |
...
| `/hirer/caregiver/:id`          | CaregiverPublicProfilePage | โปรไฟล์สาธารณะผู้ดูแล          |
| `/hirer/care-recipients`        | CareRecipientsPage         | จัดการผู้รับการดูแล             |
| `/hirer/care-recipients/new`    | CareRecipientFormPage      | เพิ่มผู้รับการดูแลใหม่          |
| `/hirer/care-recipients/:id/edit`| CareRecipientFormPage     | แก้ไขผู้รับการดูแล             |
| `/hirer/wallet`                 | HirerWalletPage            | กระเป๋าเงิน, เติม/ถอน          |
| `/hirer/wallet/receipt/:jobId`  | JobReceiptPage             | ใบเสร็จรับเงินต่องาน            |
| `/hirer/wallet/history`         | HirerPaymentHistoryPage    | ประวัติการชำระเงิน               |
...
| `/caregiver/jobs/:id/preview`     | JobPreviewPage           | ดูรายละเอียดงานก่อนรับ        |
| `/caregiver/profile`              | ProfilePage              | โปรไฟล์ผู้ดูแล                |
| `/caregiver/wallet`               | CaregiverWalletPage      | กระเป๋าเงิน, ประวัติรายได้      |
| `/caregiver/wallet/earning/:jobId`| JobEarningDetailPage     | รายละเอียดรายได้ต่องาน          |
| `/caregiver/wallet/history`       | EarningsHistoryPage      | ประวัติรายได้ทั้งหมด             |
...
| `/profile`               | ProfilePage        | ดู/แก้ไขโปรไฟล์ส่วนตัว          |
| `/settings`              | SettingsPage       | ตั้งค่าบัญชีผู้ใช้               |
...
| URL                  | หน้าจอ               | คำอธิบาย                      |
|---------------------|---------------------|-------------------------------|
| `/admin/login`      | AdminLoginPage       | Login สำหรับ Admin โดยเฉพาะ   |
...
| `/admin/reports`    | AdminReportsPage     | รายงานสรุประบบ                |
