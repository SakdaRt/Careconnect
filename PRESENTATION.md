...
**การจัดลำดับผู้ดูแล (Priority Sorting)** — ใครแสดงขึ้นก่อน:

| ลำดับ | เงื่อนไข | เหตุผล |
|:-----:|---------|--------|
| 1 | **ผู้ดูแลที่อยู่ใน Favorites ❤️** แสดงก่อน | ผู้ว่าจ้างเคยกดชอบไว้แล้ว = น่าจะอยากเห็นอีก |
| 2 | **ผู้ดูแลจริง** แสดงก่อน mock data | แยก mock/seed data ไว้ท้ายรายการ (เฉพาะ dev) |
| 3 | **Trust Score สูงสุด** แสดงก่อน | คะแนนความน่าเชื่อถือสูง = มีประวัติดี ทำงานเสร็จ รีวิวดี |
| 4 | **จำนวนงานเสร็จมากสุด** แสดงก่อน | ประสบการณ์มาก = ไว้วางใจได้ |

> SQL จริง: `ORDER BY (Favorites first) ASC, trust_score DESC, completed_jobs_count DESC`

**สรุปง่ายๆ**: ระบบจะเอาผู้ดูแลที่ผู้ว่าจ้างเคยชอบไว้ขึ้นบนสุด → ตามด้วยคนที่ Trust Score สูง → แล้วค่อยคนที่ทำงานเสร็จเยอะ

**ฟีเจอร์เสริม**:
- กด ❤️ บันทึกผู้ดูแลที่ชอบ (Favorites) — เรียกดูทีหลังได้ + ขึ้นบนสุดอัตโนมัติเวลาค้นหาครั้งต่อไป
- กดดูโปรไฟล์เต็ม — เห็น rating, รีวิว, ทักษะ, ใบรับรอง
- Direct Assign — มอบหมายงานตรงให้ผู้ดูแลที่ต้องการโดยไม่ต้องรอสมัคร

**⚠️ ยังไม่มีใน FinalReport**: รายละเอียด priority sorting 4 ลำดับ
...
**การจัดลำดับงาน (Priority Sorting)** — งานไหนแสดงขึ้นก่อน:

| ลำดับ | เงื่อนไข | เหตุผล |
|:-----:|---------|--------|
| 1 | **งานที่มอบหมายให้ตัวเอง (Direct Assign)** แสดงก่อน | ถ้า hirer เลือกผู้ดูแลคนนี้โดยเฉพาะ → ต้องเห็นก่อนเพื่อตอบรับ/ปฏิเสธ |
| 2 | **งานเร่งด่วน (is_urgent)** แสดงก่อนงานปกติ | งานที่ hirer ระบุว่าเร่ง ต้องเห็นเร็ว |
| 3 | **งานที่เริ่มเร็วที่สุด (scheduled_start_at)** แสดงก่อน | งานที่ใกล้จะถึงเวลานัด → ต้องรีบตัดสินใจ |

> SQL จริง: `ORDER BY (preferred_caregiver_id = ตัวเอง) ASC, is_urgent DESC, scheduled_start_at ASC`

**สรุปง่ายๆ**: ระบบจะเอางานที่ hirer มอบหมายให้ตัวเองขึ้นบนสุด → ตามด้วยงานเร่งด่วน → แล้วเรียงตามวันเริ่มงานที่ใกล้ที่สุด

**⚠️ ยังไม่มีใน FinalReport**: รายละเอียด 4 เงื่อนไขกรองอัตโนมัติ + priority sorting 3 ลำดับ
