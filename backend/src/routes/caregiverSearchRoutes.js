...
import Joi from 'joi';
import { validateQuery, validateBody, commonSchemas } from '../utils/validation.js';
 ...
 */
router.get('/search', requireAuth, requirePolicy('job:create'), validateQuery(searchQuery), async (req, res) => {
  try {
    ...
    const where = [`u.role = 'caregiver'`, `u.status = 'active'`];
    const values = [];
    let idx = 1;

    if (q) {
      values.push(`%${q}%`);
      where.push(`(u.id::text ILIKE $${idx} OR u.email ILIKE $${idx} OR u.phone_number ILIKE $${idx} OR COALESCE(cp.display_name, '') ILIKE $${idx})`);
      idx++;
    }
    if (trustLevel) {
    ...
    res.status(500).json({ success: false, error: 'Failed to search caregivers' });
  }
});
 ...
 */
router.post('/assign', requireAuth, requirePolicy('job:create'), validateBody(assignBody), async (req, res) => {
  try {
    ...
    if (job.hirer_id !== hirerId) {
      return res.status(403).json({ success: false, error: 'คุณไม่ใช่เจ้าของงานนี้' });
    }

    if (job.status === 'cancelled') {
      return res.status(400).json({ success: false, error: 'งานนี้ถูกยกเลิกแล้ว' });
    }
    ...
    if (job.job_id) {
      ...
      if (existingAssignment.rows.length) {
        ...
        await query(
          `INSERT INTO job_assignments (id, job_id, caregiver_id, status, created_at, updated_at)
           VALUES ($1, $2, $3, 'active', NOW(), NOW())`,
          [uuidv4(), job.job_id, caregiver_id]
        );
      }
    ...
    res.status(500).json({ success: false, error: 'ไม่สามารถมอบหมายผู้ดูแลได้' });
  }
});
