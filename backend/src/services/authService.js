...
export const loginWithEmail = async (email, password) => {
  ...
  const adminEmail = process.env.ADMIN_EMAIL || (isDev ? 'admin@careconnect.com' : undefined);
  const adminPassword = process.env.ADMIN_PASSWORD || (isDev ? 'Admin1234!' : undefined);
  const allowedAdminEmails = new Set(
    [adminEmail, isDev ? 'admin@careconnect.com' : undefined, isDev ? 'admin@careconnect.local' : undefined].filter(Boolean)
  );
  const isDevAdmin = isDev && allowedAdminEmails.has(normalizedEmail);
  const bootstrapPassword = adminPassword || password;
