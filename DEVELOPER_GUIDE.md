...
| **Testing** | Jest (backend), Playwright (E2E), k6 (load test) |
...
| **Mock Provider** (`mock-provider/src/`) | 1 ไฟล์หลัก | server.js (จำลอง Payment/SMS/KYC) |
| **Docker** | 7 ไฟล์ | docker-compose (4), Dockerfile (3) |
...
│   ├── src/
│   │   ├── config/
│   │   │   └── loadEnv.js            # โหลด environment variables จาก root .env
...
│   │   │   ├── authRoutes.js         # /api/auth/* (22 endpoints)
│   │   │   ├── jobRoutes.js          # /api/jobs/* (17 endpoints)
...
│   │   │   └── adminRoutes.js        # /api/admin/* (24 endpoints)
│   │   ├── middleware/               # ── Middleware (1 ไฟล์) ──
│   │   │   └── auth.js              # JWT verify, role check, policy gates, trust level
...
│   │   │   ├── chatSocket.js        # Socket.IO chat events (12 events)
...
| โมดูลย่อย | หน้าที่ |
|-----------|---------|
| Environment Validation | ตรวจสอบ env vars ด้วย Joi schema ก่อน start |
...
### 4.2 Routes — 17 ไฟล์ (148 endpoints)
...
|------|-----------|-----------|----------|
| `authRoutes.js` | `/api/auth` | 22 | สมัคร, login, logout, profile, avatar, password, OAuth, phone, email |
| `jobRoutes.js` | `/api/jobs` | 17 | CRUD งาน, publish, accept, checkin, checkout, cancel, stats, early-checkout |
...
| `adminRoutes.js` | `/api/admin` | 24 | user/job/dispute management, stats, trust recalculate, no-show, reports |
...
|-------------|-------|--------|----------|
| `registerGuest` | `{ email, password, role }` | `{ user, token, refreshToken }` | สมัคร guest (email) |
| `registerMember` | `{ phone_number, password, role }` | `{ user, token, refreshToken }` | สมัคร member (phone) |
| `loginWithEmail` | `{ email, password }` | `{ user, token, refreshToken }` | login ด้วย email |
| `loginWithPhone` | `{ phone_number, password }` | `{ user, token, refreshToken }` | login ด้วย phone |
| `refreshToken` | `{ refreshToken }` | `{ token, refreshToken }` | ต่ออายุ JWT |
| `logout` | `req.user` | `{ success: true }` | ลบ session |
| `getCurrentUser` | `req.user.userId` | `{ user }` | ดึงข้อมูล user ปัจจุบัน |
| `getMyProfile` | `req.user.userId` | `{ profile }` | ดึงโปรไฟล์ |
| `updateMyProfile` | `req.user.userId` + `{ display_name, bio, ... }` | `{ profile }` | แก้ไขโปรไฟล์ |
| `updateAvatar` | `req.file` (multipart, max 10MB) | `{ avatar_url }` | อัพโหลด avatar + crop/resize |
| `changePassword` | `{ current_password, new_password }` | `{ success }` | เปลี่ยนรหัสผ่าน |
| `updatePhoneNumber` | `{ phone_number }` | `{ user }` | เปลี่ยนเบอร์โทร |
| `updateEmailAddress` | `{ email }` | `{ user }` | เปลี่ยนอีเมล |
| `forgotPassword` | `{ email }` | `{ success }` | ส่ง reset link ทาง email |
| `resetPassword` | `{ token, email, new_password }` | `{ success }` | รีเซ็ตรหัสผ่านด้วย token |
| `googleLogin` | — | redirect to Google | เริ่ม Google OAuth flow |
| `googleCallback` | `?code=xxx` | redirect + JWT | รับ callback จาก Google |
| `acceptPolicyConsent` | `{ role, version_policy_accepted }` | `{ acceptance }` | ยอมรับ policy |
...
| โปรแกรมย่อย | Input | Output | Side Effects |
|-------------|-------|--------|-------------|
| `registerGuest(email, password, role)` | credentials | `{ user, tokens }` | INSERT users + profile + wallet, generate JWT |
| `registerMember(phone, password, role)` | credentials | `{ user, tokens }` | INSERT users + profile + wallet, generate JWT |
| `loginWithEmail(email, password)` | credentials | `{ user, tokens }` | verify bcrypt, INSERT auth_sessions |
| `loginWithPhone(phone, password)` | credentials | `{ user, tokens }` | verify bcrypt, INSERT auth_sessions |
| `generateTokens(user)` | user object | `{ token, refreshToken }` | sign JWT (7d access, 30d refresh) |
| `verifyRefreshToken(refreshToken)` | refresh token | `{ user }` | verify + rotate token |
| `findOrCreateGoogleUser(profile)` | Google profile | `{ user }` | find by google_id or email, create if new |
...
**`router.tsx`** — กำหนดเส้นทาง 52 routes (49 page files, บาง page ใช้ซ้ำ) ด้วย React Router v6 + lazy loading
...
AuthContext provides:
  ...
  ├── activeRole: UserRole       ← role ที่เลือกใช้งาน (hirer/caregiver)
  ├── login(credentials)         ← login + store tokens
...
|---------|-------|--------|-------------|
| `login` | `{ email/phone, password }` | `User` | store JWT + refreshToken ใน localStorage |
| `logout` | — | void | clear localStorage, redirect `/login` |
| `refreshUser` | — | `User` | GET /api/auth/me, update state |
...
ApiClient Class:
  ...
  │
  ├── request(method, path, body?)       ← core HTTP request
  ...
  └── requestFormData(method, path, formData) ← multipart upload
```
...
|-------|---------|----------|
| Auth | `registerGuest`, `registerMember`, `loginEmail`, `loginPhone`, `refreshToken`, `logout`, `getMe`, `getProfile`, `updateProfile`, `uploadAvatar`, `changePassword`, `forgotPassword`, `resetPassword` | Authentication & profile |
| Jobs | `createJob`, `publishJob`, `getJobFeed`, `getHirerJobs`, `getAssignedJobs`, `getJobDetail`, `acceptJob`, `checkIn`, `checkOut`, `cancelJob`, `uploadCheckoutPhoto`, `requestEarlyCheckout`, `respondEarlyCheckout` | Job lifecycle |
| Wallet | `getBalance`, `getTransactions`, `getBankAccounts`, `addBankAccount`, `createTopup`, `createWithdrawal`, `cancelWithdrawal` | Financial |
| Chat | `getThreads`, `getMessages`, `sendMessage`, `uploadChatImage`, `markAsRead` | Messaging |
...
| OTP | `sendEmailOtp`, `sendPhoneOtp`, `verifyOtp` | OTP verification |
| KYC | `getKycStatus`, `submitKyc` | Identity verification |
| Disputes | `createDispute`, `getDisputeDetail`, `postDisputeMessage`, `uploadDisputeImage` | Dispute system |
...
| `checkOut(jobId, data)` | `api.checkOut` | checkout with evidence |
...
- **Migrations**: `database/migrations/` (15 ไฟล์ incremental)
...
└─────────────────────────────────────────────────────────────────────┘
  ...
  │  L1: phone OR email verified             │
  │  L2: phone verified + KYC approved       │
  │  L3: phone + KYC + bank + score ≥ 80     │
...
### 10.1 Frontend → Backend API Communication

#### Request Format (ทุก API call)
...
HTTP Request:
  ...
  Query: ?key=value                     // for GET with filters
```
...
// Error
{
  ...
  "error": {
    ...
    "message": "ข้อมูลไม่ถูกต้อง",  // Thai error message
    "details": [                   // optional validation details
      { "field": "email", "message": "จำเป็นต้องระบุอีเมล" }
    ]
  }
}
```
...
|------|------------|----------|
| `VALIDATION_ERROR` | 400 | ข้อมูลไม่ผ่าน Joi validation |
| `UNAUTHORIZED` | 401 | ไม่มี/ไม่ถูกต้อง JWT token |
| `FORBIDDEN` | 403 | ไม่มีสิทธิ์ (role/trust/policy/ban) |
| `NOT_FOUND` | 404 | ไม่พบ resource |
| `CONFLICT` | 409 | ข้อมูลซ้ำ (email ซ้ำ, job ถูกรับแล้ว) |
...
JWT Payload (Access Token):
{
  ...
  "role": "hirer|caregiver|admin", // user role
  "accountType": "guest|member",   // account type
  "trustLevel": "L0|L1|L2|L3",    // trust level
...
{
  "userId": "uuid-v4",
  "tokenId": "uuid-v4",            // session ID
  "type": "refresh",
  "iat": 1234567890,
  "exp": 1234567890                // expires (30 days)
}
```

### 10.3 พารามิเตอร์สำคัญระหว่างโมดูล

#### 10.3.1 Route → Controller Parameters

```
Controller receives from Express:
  ...
  req.query = {                   // Query string (paginated lists)
...
#### 10.3.2 Controller → Service Parameters

```
// Example: jobController → jobService

createJob(userId, {
  title: string,
  description?: string,
  job_type: 'elderly_care'|'escort'|'post_hospital'|'special_care',
  care_tasks: string[],
  required_skills: string[],
  hourly_rate: number,           // บาท/ชั่วโมง
...
// Returns: { job_post } with auto-computed:
//   risk_level: 'low_risk'|'high_risk'
//   min_trust_level: 'L1'|'L2'
//   risk_reason_codes: string[]
//   risk_reason_detail: string
//   total_amount: number (hourly_rate × hours)
//   hirer_deposit: number (depositTier)
```

#### 10.3.3 Service → Model Parameters

```
// Example: jobService → Job model

Job.create({
  hirer_id: string,              // UUID
  title: string,
  job_type: string,
  risk_level: string,
...
// Model executes:
//   INSERT INTO job_posts (...) VALUES ($1, $2, ...) RETURNING *
//   Returns: row object (all columns)
```

#### 10.3.4 Service → Service Parameters

```
// jobService → walletService (on publish)
walletService.holdJobPayment(
  userId: string,                 // hirer UUID
  totalAmount: number,            // job cost
  hirerDeposit: number,           // deposit amount
  jobPostId: string               // job_post UUID
)

// jobService → walletService (on checkout/settlement)
walletService.releaseEscrowToCaregiver(
  escrowWalletId: string,         // escrow wallet UUID
  caregiverId: string,            // caregiver UUID
  payoutAmount: number,           // amount after fee
  platformFee: number,            // 10% fee
  hirerDeposit: number,           // deposit to release
  hirerId: string,                // hirer UUID (deposit recipient)
  jobPostId: string               // for reference
)

// jobService → notificationService (on accept)
notificationService.notifyJobAccepted(
  hirerId: string,                // target user
  jobTitle: string,               // job title text
  caregiverName: string           // CG display name
)

// jobService → trustLevelWorker (on checkout)
trustLevelWorker.triggerUserTrustUpdate(
  caregiverId: string,            // target user
  reason: 'job_completed'|'no_show'|'cancellation'|'review_received'
)
```

#### 10.3.5 Socket.IO Event Parameters
...
// Client → Server events
{
  ...
  'message:send':   {
    ...
    attachment_key?: string      // from upload API
  }
  'typing:start':   { threadId: string }
  'typing:stop':    { threadId: string }
  'message:read':   { threadId: string, messageId: string }
}

// Server → Client events
{
  'thread:joined':   { threadId: string }
  'message:new':     {
    id: string,
    thread_id: string,
    sender_id: string,
  ...
  'typing:started':  { threadId: string, userId: string }
  'typing:stopped':  { threadId: string, userId: string }
  'notification:new': {
    id: string,
    template_key: string,
    title: string,
...
Wallet Types (wallets.wallet_type):
  ├── 'hirer'      — ผู้ว่าจ้าง (1 per user)
  ├── 'caregiver'  — ผู้ดูแล (1 per user)
  ├── 'escrow'     — เงินค้ำประกัน (1 per active job)
  ├── 'platform'   — ค่าธรรมเนียมระบบ (singleton)
  └── 'deposit'    — เงินประกัน (1 per active job)

Ledger Transaction Types (ledger_transactions.type):
  ├── 'credit'   — เพิ่มเงิน (topup, refund)
  ├── 'debit'    — ลดเงิน (withdraw, fee)
  ├── 'hold'     — กันเงิน (publish job)
  ├── 'release'  — ปล่อยเงิน (settlement, cancel refund)
  └── 'transfer' — โอนระหว่าง wallets

Reference Types (ledger_transactions.reference_type):
  ├── 'topup'       — topup intent
  ├── 'withdrawal'  — withdrawal request
  ├── 'job'         — job payment
  ├── 'deposit'     — hirer deposit
  ├── 'fee'         — platform fee
  ├── 'compensation'— CG compensation (late cancel)
  ├── 'penalty'     — penalty revenue
  ├── 'refund'      — cancel refund
  └── 'admin'       — admin action
```

### 10.5 Job Status State Machine

```
job_posts.status:
  draft ──publish──▶ posted ──accept──▶ assigned* ──▶ (managed via jobs table)
    │                   │
    │                   └──cancel──▶ cancelled
    └──delete──▶ (removed)

  * เมื่อ caregiver accept จะสร้าง row ใน jobs table

jobs.status:
  assigned ──checkin──▶ in_progress ──checkout──▶ completed
     │                      │
     │                      └──cancel──▶ cancelled
     └──cancel/noshow──▶ cancelled
...
early_checkout_requests.status:
  pending ──approve──▶ approved ──▶ (trigger checkout)
    │
    └──reject──▶ rejected
    │
    └──(1hr auto)──▶ auto_approved ──▶ (trigger checkout)
```
...
docker-compose.yml (Development):
  ...
  │────────────────│────────────────│───────│──────────────│
  │ frontend       │ Dockerfile.dev │ 5173  │ backend      │
  │ backend        │ Dockerfile.dev │ 3000  │ postgres     │
  │ postgres       │ postgres:15    │ 5432  │ —            │
  │ mock-provider  │ Dockerfile.dev │ 4000  │ backend      │
  └────────────────────────────────────────────────────────┘

  Networks: careconnect (bridge)
...
docker-compose.prod.yml (Production):
  ...
  │ Service        │ Image/Build    │ Port  │ Depends On   │
  │────────────────│────────────────│───────│──────────────│
  │ frontend       │ Dockerfile     │ 80    │ backend      │
  │ backend        │ Dockerfile     │ 3000  │ postgres     │
  │ postgres       │ postgres:15    │ 5432  │ —            │
  └────────────────────────────────────────────────────────┘
```
...
docker compose exec backend node src/seeds/runDemoSeed.js
...
| Backend Routes | 17 | 148 endpoints |
...
| Frontend Layouts | 4 | 4 layout types |
| Frontend Services | 2 | 80+ API methods |
| Frontend Contexts | 1 | auth state |
| Frontend Utils | 9 | ~25 helpers |
| Frontend Router | 2 | 52 routes + 5 guards |
| Database Schema | 1 | 41 tables |
| Database Migrations | 15 | incremental changes |
| Mock Provider | 1 | 4 simulation endpoints |
| Docker Config | 7 | 4 compose files |
...
> เอกสารนี้สร้างโดย AI Assistant เมื่อ 2026-04-04
