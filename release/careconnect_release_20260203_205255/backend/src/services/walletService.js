...
class WalletService {
  ...
  async addBankAccount(userId, bankAccount) {
    ...
    return await transaction(async (client) => {
      ...
      if (isVerified) {
        await client.query(
          `UPDATE users SET trust_level = 'L3', updated_at = NOW()
           WHERE id = $1 AND trust_level <> 'L3'`,
          [userId]
        );
      }
  ...
  async initiateWithdrawal(userId, role, amount, bankAccountId) {
    ...
    if (trustLevel !== 'L3') {
      throw { status: 403, message: `Withdrawal requires Trust Level L3. Your level: ${trustLevel}` };
    }
