...
async function verifyOtp(otpId, code) {
  ...
  try {
    ...
    // Check if user should be upgraded to L1
    ...
    if (userResult.rows[0]) {
      ...
      if ((user.is_email_verified || user.is_phone_verified) && user.trust_level === 'L0') {
        await query(
          `UPDATE users SET trust_level = 'L1', updated_at = NOW() WHERE id = $1`,
          [otpData.userId]
        );
        console.log(`[OTP Service] User ${otpData.userId} upgraded to L1`);
      }
    }
